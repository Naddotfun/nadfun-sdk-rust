...
- **BREAKING:** removed the baked LvMON quote-token constants — the
  ...
  `V2CreatePayment::Native { quote_token: addr }`. The v1 `get_wmon` /
...
- `src/types/mod.rs` and `src/lib.rs` switched from `pub use *` glob to
  ...
  root (Codex P1 #7).
- **`CoreV2` removed.** Use the `core.v2()` namespace handle. See
  [`MIGRATION.md`].
...
- `V2BuyWithNativeParams` gains a `value` field; `core.v2().buy_with_native`
...
- v2 event decoder reads indexed fields from topics via
  ...
  (Codex P1 #1).
- `core.v2().create_token` waits for the receipt and asserts the on-chain
...
- `estimate_gas` (v1) rejects `Address::ZERO` as `to` (which doubles as
  the from-address for the estimate); `core.v2().estimate_gas` errors
...
- `discover_pools_unified` uses `TokenRegistryV2::getPair` instead of
  ...
  (Codex P2 #11).
- `core.v2().create_token` uses server-normalized name/symbol from
...
### Added — unified `Core` + v2 contract support

- **Unified `Core` (v1 + v2 from one instance) + v2 contract support.**
  The SDK ships a **single `Core`** that wires both the v1 (bonding curve
  + Capricorn CL DEX) and v2 (NadFunRouter + per-token registry + vaults)
  contract surfaces, reached through the `core.v1()` / `core.v2()` namespace
  handles. The process-global `set_network` lock is gone — every entry point
  binds to a `Network` at construction.

- **`Core`** absorbs the v0.3 `CoreV2`. v2 surface (via `core.v2()`):
  ...
  - Creation: `create`, `create_with_native` (low-level), and
    `create_token(V2CreateTokenParams, &ApiClient)` (end-to-end). The
    high-level path now verifies the on-chain `Create` event matches the
    predicted token address.
  - Quotes (price): `get_amount_out`, `get_amount_in`,
    `get_bonding_curve_amount_out(_in)`, `get_dex_amount_out(_in)`.
    Renamed from `quote_*` so the word "quote" is reserved for "quote
    token" (the trade's pricing currency, e.g. WMON / LvMON / USDT).
  - Pool / state: `is_graduated`, `pool_address`, `wrapped_native`.
  - Gas: `estimate_gas(V2GasEstimationParams)`.
  - Escape hatches: `core.v2().router()`, `.factory()`, `.bonding_curve()`,
    `.token_registry()` — return `&_` directly. v2 is always wired
...
- **`ApiClient`** v2 surface:
  ...
  - `SaltParams.version: SdkVersion` — `V1` omits the field on the wire
    (`skip_serializing_if = "is_v1"`); `V2` enables v2 CREATE2 mining.
